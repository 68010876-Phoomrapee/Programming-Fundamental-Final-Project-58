...
// and length between 1 and 20 (practical bound)
...
// validate date "DD/MM/YYYY", range DD valid per month, year 1950..2100
...
int input_line(char *prompt, char *buf, int bufsize) {
    while (1) {
        ...
        // trim newline
        size_t L = strlen(buf);
        if (L>0 && (buf[L-1]=='\n' || buf[L-1]=='\r')) { buf[L-1]=0; --L; if (L>0 && buf[L-1]=='\r') { buf[L-1]=0; --L; } }
...
void display_all() {
    ...
    printf("\n---- All inspections (%d) ----\n", n);
    printf("%-12s | %-12s | %-20s | %-12s\n", "InspectionID", "CarReg", "Owner", "Date");
    printf("---------------------------------------------------------------\n");
    for (int i = 0; i < n; ++i) {
        printf("%-12s | %-12s | %-20s | %-12s\n",
...
// helper: find index by inspectionID or carReg (exact match); return -1 if not found
int find_by_id_or_reg(Record arr[], int n, const char *key) {
    for (int i = 0; i < n; ++i) {
        if (strcmp(arr[i].inspectionID, key) == 0) return i;
        if (strcmp(arr[i].carReg, key) == 0) return i;
    }
    return -1;
}
...
void add_record() {
    Record arr[MAX_RECORDS];
    int n = load_all(arr, MAX_RECORDS);
    if (n >= MAX_RECORDS) { printf("Max records reached.\n"); return; }
    ...
    while (1) {
        ...
        if (!is_valid_id_or_reg(buf)) { printf("Invalid ID format. Use letters and digits only (1-20 chars).\n"); continue; }
        // ensure unique
        if (find_by_id_or_reg(arr, n, buf) != -1) { printf("This InspectionID (or CarReg) already exists. Choose another.\n"); continue; }
        strncpy(r.inspectionID, buf, sizeof(r.inspectionID)-1); r.inspectionID[sizeof(r.inspectionID)-1]=0;
        break;
    }
    ...
    while (1) {
        ...
        if (!is_valid_id_or_reg(buf)) { printf("Invalid CarReg format. Use letters and digits only.\n"); continue; }
        // ensure unique
        if (find_by_id_or_reg(arr, n, buf) != -1) { printf("This InspectionID (or CarReg) already exists. Choose another.\n"); continue; }
        strncpy(r.carReg, buf, sizeof(r.carReg)-1); r.carReg[sizeof(r.carReg)-1]=0;
        break;
    }
    ...
    while (1) {
        ...
        if (!is_valid_owner_name(buf)) { printf("Invalid owner name. Use letters and spaces only (1-60 chars).\n"); continue; }
        strncpy(r.owner, buf, sizeof(r.owner)-1); r.owner[sizeof(r.owner)-1]=0;
        break;
    }
    ...
    while (1) {
        ...
        if (!is_valid_date(buf)) { printf("Invalid date. Ensure format DD/MM/YYYY and valid day/month/year range.\n"); continue; }
        strncpy(r.date, buf, sizeof(r.date)-1); r.date[sizeof(r.date)-1]=0;
        break;
    }
    ...
    char c = getchar();
    while (getchar() != '\n'); // clear rest
    if (c != 'Y' && c != 'y') { printf("Add cancelled.\n"); return; }
...
void search_record() {
    ...
    // print matches (we use strstr for partial match on owner? but requirement is search by ID or CarReg; we'll match exact or substring)
    int found = 0;
    for (int i = 0; i < n; ++i) {
        if (strcmp(arr[i].inspectionID, buf) == 0 || strcmp(arr[i].carReg, buf) == 0 ||
            strstr(arr[i].inspectionID, buf) || strstr(arr[i].carReg, buf)) {
...
void update_record() {
    ...
    if (idx == -1) { printf("No exact match found for '%s'. Try search first.\n", buf); return; }
    ...
    // trim newline
    size_t L = strlen(in); if (L>0 && (in[L-1]=='\n' || in[L-1]=='\r')) in[--L]=0;
    if (strcmp(in, "0")==0) return;
    if (strlen(in) > 0) {
        if (!is_valid_id_or_reg(in)) { printf("Invalid format; update aborted.\n"); return; }
        // ensure unique (except itself)
        int conflict = find_by_id_or_reg(arr, n, in);
        if (conflict != -1 && conflict != idx) { printf("ID conflict; update aborted.\n"); return; }
        strncpy(arr[idx].inspectionID, in, sizeof(arr[idx].inspectionID)-1); arr[idx].inspectionID[sizeof(arr[idx].inspectionID)-1]=0;
    }

    // CarReg
    printf("New CarReg (enter to keep current '%s', or 0 to back): ", arr[idx].carReg);
    if (!fgets(in, sizeof(in), stdin)) return;
    L = strlen(in); if (L>0 && (in[L-1]=='\n' || in[L-1]=='\r')) in[--L]=0;
    if (strcmp(in, "0")==0) return;
    if (strlen(in) > 0) {
        if (!is_valid_id_or_reg(in)) { printf("Invalid format; update aborted.\n"); return; }
        int conflict = find_by_id_or_reg(arr, n, in);
        if (conflict != -1 && conflict != idx) { printf("CarReg conflict; update aborted.\n"); return; }
        strncpy(arr[idx].carReg, in, sizeof(arr[idx].carReg)-1); arr[idx].carReg[sizeof(arr[idx].carReg)-1]=0;
    }
    ...
    if (strcmp(in, "0")==0) return;
    if (strlen(in) > 0) {
        if (!is_valid_owner_name(in)) { printf("Invalid owner name; update aborted.\n"); return; }
        strncpy(arr[idx].owner, in, sizeof(arr[idx].owner)-1); arr[idx].owner[sizeof(arr[idx].owner)-1]=0;
    }
    ...
    if (strcmp(in, "0")==0) return;
    if (strlen(in) > 0) {
        if (!is_valid_date(in)) { printf("Invalid date; update aborted.\n"); return; }
        strncpy(arr[idx].date, in, sizeof(arr[idx].date)-1); arr[idx].date[sizeof(arr[idx].date)-1]=0;
    }
...
void delete_record() {
    ...
    int idx = -1;
    for (int i = 0; i < n; ++i) {
        if (strcmp(arr[i].inspectionID, buf) == 0) { idx = i; break; }
    }
    if (idx == -1) { printf("No exact match found for '%s'.\n", buf); return; }

    printf("Found: %s | %s | %s | %s\n", arr[idx].inspectionID, arr[idx].carReg, arr[idx].owner, arr[idx].date);
    printf("Confirm delete? (Y/N): ");
    char c = getchar();
    while (getchar() != '\n'); // clear
    if (c != 'Y' && c != 'y') { printf("Delete cancelled.\n"); return; }
    // remove by shifting
...
int main() {
    ...
    while (1) {
        ...
        printf("Choose option: ");
        char optbuf[16];
        if (!fgets(optbuf, sizeof(optbuf), stdin)) break;
        int opt = atoi(optbuf);
        if (opt == 0 && strcmp(optbuf,"0\n")!=0 && strcmp(optbuf,"0\r\n")!=0) {
            // if user entered non-numeric or blank, ignore
            continue;
        }
        switch (opt) {
